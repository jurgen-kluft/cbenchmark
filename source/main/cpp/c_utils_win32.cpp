...
namespace BenchMark
{
	#define DEST_S(dest, destEnd) dest, (int)(dest - dstEnd)

	char* gStringFormatAppend(char* dest, const char* dstEnd, const char* format, const void* p) { int a = sprintf_s(DEST_S(dest, dstEnd), format, p); return dest + a; }
